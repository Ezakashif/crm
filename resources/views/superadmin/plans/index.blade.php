...
<div class="sa-card">
    <form method="GET" class="form-row align-items-end">
        ...
        <div class="form-group col-md-4 mb-0"><button class="btn btn-outline-light mr-2">Filter</button><a class="btn btn-info" href="{{ route('superadmin.plans.create') }}">New plan</a></div>
    </form>
</div>
<div class="sa-card">
    <div class="table-responsive"><table class="table mb-0"><thead><tr><th>Plan</th><th>Monthly</th><th>Yearly</th><th>Trial</th><th>Features</th><th>Limits</th><th>Status</th><th>Visibility</th><th>Created</th><th>Actions</th></tr></thead><tbody>
    @forelse ($plans as $plan)
        <tr><td><strong class="text-white">{{ $plan->name }}</strong><div class="sa-muted small">{{ $plan->slug }}</div></td><td>{{ $plan->currency }} {{ number_format((float) $plan->monthly_price, 2) }}</td><td>{{ $plan->currency }} {{ number_format((float) $plan->yearly_price, 2) }}</td><td>{{ $plan->trial_days }} days</td><td>{{ $plan->features_count }}</td><td>{{ $plan->limits_count }}</td><td><span class="badge badge-{{ $plan->is_active ? 'active' : 'suspended' }}">{{ $plan->is_active ? 'Active' : 'Inactive' }}</span></td><td>{{ $plan->is_public ? 'Public' : 'Hidden' }}</td><td class="sa-muted">{{ $plan->created_at?->format('Y-m-d') }}</td><td><a class="btn btn-sm btn-outline-light" href="{{ route('superadmin.plans.edit', $plan) }}">Edit</a></td></tr>
    @empty
        <tr><td colspan="10"><div class="sa-empty"><h2 class="sa-empty__title">No subscription plans found</h2><p class="sa-empty__text">Create a plan to control pricing, features, and limits.</p></div></td></tr>
    @endforelse
    </tbody></table></div>
